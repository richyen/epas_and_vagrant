...
### Set up EFM
rpm -ivh http://yum.enterprisedb.com/reporpms/enterprisedb-tools-repo-1.0-1.noarch.rpm
sed -i "s/<username>:<password>/${YUMUSERNAME}:${YUMPASSWORD}/" /etc/yum.repos.d/enterprisedb-tools.repo

yum install -y which efm20 java-1.6.0-openjdk
...
cp /vagrant/efm.properties /etc/efm-2.0/efm.properties
cp /etc/efm-2.0/efm.nodes.in /etc/efm-2.0/efm.nodes
